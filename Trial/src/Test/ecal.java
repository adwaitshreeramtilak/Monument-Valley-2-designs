...
public class ecal {
	public int xcorwrto(int x) {
		x = x - 678;
		return x;
	}
	public int ycorwrto(int y) {
		y = 360 - y;
		return y;
	}
	public int orx(int x) {
		x = x + 678;
		return x;
	}
	public int ory(int y) {
		y = 360 - y;
		return y;
	}
	public int rotx(int x, int y, double theta) {
		double unrans = (x*(Math.cos(theta)))-(y*(Math.sin(theta)));
		int ans = (int) Math.round(unrans);
		return ans;
	}
	public int roty(int x, int y, double theta) {
